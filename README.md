...
This repository contains the NARA marketing site with three primary experiences:

- Provider landing page (index)
- Organization landing page under `/organizations`
...
- `/organizations` – Organization landing page (see `components/home/organization-homepage.tsx`)
...
- **Animations**: Framer Motion is used throughout with `initial`, `whileInView`, `transition`, and `viewport` props for appear-on-scroll effects. Prefer `easeOut` and short durations (200–500ms). Stagger delays (e.g., `0.08 * index`) create sequential entrance animations for lists. All page components are client-side (`"use client"`) to support `whileInView`.
- **JSX Structure**: ensure all `SectionContainer`, `motion.div`, and `div` elements have matching closing tags. The organization homepage was recently refactored to fix unbalanced JSX and restore proper nesting.
...
- **Footer**: Standardized at the page level for provider, organization, and client routes.
