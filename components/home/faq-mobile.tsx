        ...
        return (
          <div key={item.q} className="rounded-lg border border-[#d0d5dd] bg-white">
            <button
              ...
              aria-expanded={open}
            >
              <p className="flex-1 text-[14px] font-medium text-[#101828]">{item.q}</p>
              {open ? (
                <ArrowDown2 size={18} color="#667085" />
              ) : (
                <ArrowRight2 size={18} color="#667085" />
              )}
            </button>
            {open && item.a && (
              <p className="px-3 pb-4 text-[14px] text-[#667085]">{item.a}</p>
            )}
          </div>
        );
      })}
